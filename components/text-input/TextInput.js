...
var TextInput = function (_Component) {
  ...
  TextInput.prototype.componentDidUpdate = function componentDidUpdate(prevProps, prevState) {
    ...
    if (!focused && prevState.focused) {
      _utils.KeyboardAccelerators.stopListeningToKeyboard(this, focusedKeyboardHandlers);
    }

    if (!dropActive && prevState.dropActive) {
      document.removeEventListener('click', this.onRemoveDrop);
      _utils.KeyboardAccelerators.stopListeningToKeyboard(this, activeKeyboardHandlers);
      if (this.drop) {
        this.drop.remove();
        this.drop = undefined;
      }
    ...
    if (focused && !prevState.focused) {
      _utils.KeyboardAccelerators.startListeningToKeyboard(this, focusedKeyboardHandlers);
    }

    if (dropActive && !prevState.dropActive) {
      document.addEventListener('click', this.onRemoveDrop);
      _utils.KeyboardAccelerators.startListeningToKeyboard(this, activeKeyboardHandlers);

      // If this is inside a FormField, place the drop in reference to it.
      // TODO: fix form field
      // findAncestor(this.componentRef, FORM_FIELD)
      var control = this.componentRef;
      this.drop = new _utils.Drop((0, _reactDom.findDOMNode)(control), this.renderDropContent(), {
